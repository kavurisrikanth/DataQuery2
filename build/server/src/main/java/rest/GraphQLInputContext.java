...
public abstract class GraphQLInputContext {
	...
	public GraphQLInputContext(EntityHelperService helperService, Map<Long, Object> inputObjectCache, Map<String, DFile> files,
			JSONObject variables) {
	...
	protected <T> T readRef(EntityHelper<T, ? extends IEntityInput> helper, long id) {
		if (id > 0) {
			...
			if (obj == null) {
				throw new ValidationFailedException(MutateResultStatus.BadRequest,
						ListExt.asList("Nothing found for current id."));
			}
			return obj;
		}
		if (id == 0) {
